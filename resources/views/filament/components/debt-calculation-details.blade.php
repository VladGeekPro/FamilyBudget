...
if ($overpayment) {
    ...
    if ($minUser->user_id === $overpayment->user_id) {
        ...
        $adjustmentNote = "К долгу " . number_format($baseDifference, 2, ',', ' ') . " MDL пользователя {$minUser->user->name} добавлена договорная сумма переплаты " . number_format($overpaymentSum, 2, ',', ' ') . " MDL";
    } else {
        ...
        if ($finalDifference < 0) {
            ...
            $adjustmentNote = "{$overpayment->user->name} должен переплатить по договорённости " . number_format($overpaymentSum, 2, ',', ' ') . " MDL — эта сумма превышает его долг, поэтому теперь {$minUser->user->name} должен(на) {$maxUser->user->name}";
        } else {
            $adjustmentNote = "Переплата " . number_format($overpaymentSum, 2, ',', ' ') . " MDL пользователем {$overpayment->user->name} вычтена из его долга, должником остаётся {$minUser->user->name}";
        }
    }
}
...
$finalDiffFormatted = number_format($finalDifference, 2, ',', ' ');
        ...
        <div class="w-full">
            <details open class="bg-gradient-to-br from-white to-gray-50 dark:from-gray-900 dark:to-gray-800 rounded-lg border-2 border-gray-200 dark:border-gray-700 transition-all duration-300 overflow-hidden">
                ...
                <div class="p-3 space-y-3 sm:space-y-5">
                        ...
                        <div class="relative z-10">
                            ...
                            <div class="space-y-2 sm:space-y-3">
                                ...
                                <div class="flex items-center gap-2 sm:gap-3 py-2 sm:py-3">
                                    ...
                                    <div class="px-3 py-1 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-lg border border-amber-200 dark:border-amber-800">
                                ...
                                <div class="bg-gradient-to-r from-cyan-50 to-blue-50 dark:from-cyan-950/40 dark:to-blue-950/40 rounded-lg p-3 border-2 border-cyan-300/50 dark:border-cyan-700/50 shadow-sm">
                                    <div class="flex gap-2 sm:gap-3">
                                        ...
                                        <p class="text-xs sm:text-sm font-medium text-cyan-900 dark:text-cyan-100 leading-relaxed">{{ $adjustmentNote }}</p>
                                    </div>
                                </div>
                                @endif
                            </div>
                            ...
                            <!-- Итоговая карточка -->
                            <div class="mt-4 sm:mt-6 pt-4 sm:pt-5 border-t-2 border-dashed border-amber-300 dark:border-amber-700">
                                <div class="relative bg-gradient-to-r from-amber-500 via-orange-500 to-red-500 rounded-lg p-3 text-white shadow-2xl overflow-hidden">
                    ...
                    <!-- Детали затрат -->
                    <details class="group/expenses bg-white dark:bg-gray-800 rounded-lg border-2 border-gray-200 dark:border-gray-700 shadow-md overflow-hidden">
                        ...
                        <div class="p-3 space-y-2 sm:space-y-3 bg-gray-50 dark:bg-gray-900/30">
                            ...
                            <details class="group/user bg-white dark:bg-gray-800 rounded-lg border-l-4 border-amber-400 dark:border-amber-600 shadow-md overflow-hidden">
                                <summary class="cursor-pointer px-3 py-3 hover:bg-gray-50 dark:hover:bg-gray-750 transition-colors duration-150 flex items-center gap-2 sm:gap-3 active:scale-[0.98]">
                                    <div class="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-gradient-to-br from-amber-100 to-orange-100 dark:from-amber-900/30 dark:to-orange-900/30 flex items-center justify-center flex-shrink-0">
                                        <svg class="w-4 h-4 sm:w-5 sm:h-5 text-amber-600 dark:text-amber-400" fill="currentColor" viewBox="0 0 20 20">
                                    ...
                                    <div class="hidden sm:block overflow-x-auto">
                                        <table class="w-full text-xs sm:text-sm">
                                            ...
                                            <tbody class="divide-y divide-gray-200 dark:divide-gray-700">
                                                ...
                                                $expSum = number_format($expense->sum, 2, ',', ' ');
                                                @endphp
                                                <tr class="hover:bg-white dark:hover:bg-gray-800 transition-colors duration-100">
